...
def get_Jaccard_Similarity(interaction_matrix):
    ...
    E = np.ones_like(X.T)                                       #返回与 a 相同形状和类型的数组,是1填充的
    denominator=X * E + E.T * X.T - X * X.T
    denominator_zero_index=np.where(denominator==0)             #返回符合条件的值的索引
...
def get_CommonNeighbours_Similarity(interaction_matrix):        #公共邻居算法
    X = np.mat(interaction_matrix)
...
def get_Salton_Similarity(interaction_matrix):                  #扩展的布尔检索模型 ：将向量检索与布尔检索融为一体
    X = np.mat(interaction_matrix)
