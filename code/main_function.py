...
def indentTraTes(opt):
    ...
    ######创建文件夹
    ######path = r'D:\pycharm\Drugbank Data Process\MVGCN\MVGCN-main\Methods\code\output\\'
    epoch=opt.epochs
    ...
    fileName = opt.resultTxt + '/label_score_kfold'+str(kfold)+'.csv'
    #print(fileName)
    write_score(test_label, test_score, fileName, opt)

    result = np.zeros((1,8))
    result[0,1:] = np.array(criteria_result)
    resultTxt = resultTxt+'/All.csv'
    write_result(result, resultTxt, opt)
